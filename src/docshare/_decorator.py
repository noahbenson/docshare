...
from ._inherit import _TEXT_TARGET, DESCRIPTION, SUMMARY, Operation, compose
...
__all__ = ('doccompose', 'docrender', 'docwrap')
...
def custom_arguments(declared, caller='docwrap'):
    ...
    caller : str, optional
        The name of the function the caller used, for error messages.
        ...
        for name in names:
            if name in SECTION_ARGUMENTS or name in GENERAL_ARGUMENTS:
                raise DocShareError(
                    ...
                    f'{name}=, which {caller} already accepts for something '
...
def _unknown_argument(name, custom_args=None, caller='docwrap'):
    ...
    if intended is not None:
        ...
        raise DocShareError(
            f'{caller}() got an unexpected argument {name!r}; use '
    ...
    raise DocShareError(
        f'{caller}() got an unexpected argument {name!r}{hint}'
    )
...
def _declaration(options, caller='docwrap'):
    """Normalize a call's section declaration and check its arguments.

    What the call declares decides which arguments it may use, so the
    declaration is read before the arguments are checked; a name that is
    neither a general argument, a per-section argument, nor one generated by
    the declaration is rejected.
    """
    declared = normalize_custom(options.get('custom'))
    custom_args = custom_arguments(declared, caller)
    for name in options:
        if (
            ...
            _unknown_argument(name, custom_args, caller)
    return declared, custom_args


def _assemble(obj, options, target, accept_document, caller='docwrap'):
    """Parse, validate, and compose a target's documentation.

    The order matters: the declaration is read first, the target is parsed,
    it is validated against the object before anything is pulled from a
    source, how each source is read is settled before any source is read,
    and only then is anything composed. Returns the declaration, the parsed
    target document, and the composed document, or ``None`` for that last
    when the call asked for nothing and the target is left as it stands.

    Parameters
    ----------
    obj : object
        The documented object, a docstring, or an already-parsed `Document`.
    options : mapping
        The arguments the caller gave.
    target : object
        The object whose signature orders and validates the documentation,
        or `_TEXT_TARGET` when there is none.
    accept_document : bool
        Whether `obj` may be a `Document` whose parse is skipped.
    caller : str, optional
        The name of the function the caller used, for error messages.
    """
    declared, custom_args = _declaration(options, caller)
    format = options.get('format')
    extraparam = options.get('extraparam')
    if accept_document and isinstance(obj, Document):
        doc = obj
    else:
        doc = docparse(obj, format=format, custom=options.get('custom'))
    if target is not _TEXT_TARGET:
        validate_signature(
            target,
            doc,
    ...
    composed = compose(target, doc, operations, extraparam=extraparam)
    ...
    # document is checked before it is used.
    if target is not _TEXT_TARGET:
        validate_signature(
            target,
            composed,
    ...
    return declared, doc, composed


def _apply(obj, options):
    """Compose and install documentation on a single object."""
    declared, doc, composed = _assemble(obj, options, obj, False)
    format = options.get('format')
...
def _standalone(obj, options, caller):
    """Assemble a call of `doccompose` or `docrender`, without an object."""
    if obj is None:
        raise DocShareError(
            f'{caller}() needs documentation to work on: give a docstring, '
            f'an object, or a Document'
        )
    target = _TEXT_TARGET if isinstance(obj, (str, Document)) else obj
    return _assemble(obj, options, target, True, caller)


def doccompose(obj, /, **options):
    """Compose documentation without decorating anything.

    This is `docwrap` with nothing to install the result on: it reads the
    documentation given, performs the inheritance the arguments ask for, and
    returns the composed `Document` rather than replacing an object's
    ``__doc__``. The documentation given is parsed afresh, so nothing is read
    from or written to the cache; a source is still read through `docinfo`,
    as it is for `docwrap`.

    Parameters
    ----------
    obj : str, object, or Document
        The documentation to compose. A string is taken to *be*
        documentation rather than to have any; an object is documented by its
        ``__doc__``, and its signature orders and validates the
        documentation; an already-parsed `Document` is composed as it stands.
        A string or a `Document` has no signature, so no validation is
        attempted and a section of parameters is ordered by the documentation
        itself and its sources.
    **options
        Any argument `docwrap` accepts, meaning the same thing there. See
        `docwrap` for the vocabulary.

    Returns
    -------
    Document
        The composed document: the target's own documentation, with whatever
        its sources supplied for what it left out. When no inheritance was
        asked for, it is the documentation as parsed.

    Raises
    ------
    DocShareError
        If `obj` is ``None``, or an argument is not recognized or is given in
        two places.
    DocFormatError
        If the format is unsupported or cannot be detected.
    DocInheritanceError
        If the requested inheritance is incoherent.
    DocMappingError
        If a mapping or a bound source is invalid.
    """
    _, doc, composed = _standalone(obj, options, 'doccompose')
    return doc if composed is None else composed


def docrender(obj, /, **options):
    """Compose and render documentation without decorating anything.

    This is `docwrap` with nothing to install the result on: it reads the
    documentation given, performs the inheritance the arguments ask for, and
    returns the rendered docstring rather than replacing an object's
    ``__doc__``. The result is ordinary documentation text, suitable for
    assignment to a ``__doc__`` or for any other use.

    Rendering normalizes presentation --- indentation, the blank lines
    between sections, and the spelling of a recognized section's title --- so
    the result is not always identical to the documentation given, even when
    nothing was inherited. Like `doccompose`, it parses the documentation
    afresh and writes nothing to the cache.

    Because it assigns nothing, `docrender` also works on objects whose
    ``__doc__`` cannot be written --- a class, a `property`, a
    `classmethod` --- where `docwrap` reports an error.

    Parameters
    ----------
    obj : str, object, or Document
        The documentation to compose and render, as `doccompose` accepts it.
        An object's signature orders and validates the documentation; a
        string or a `Document` has no signature, so no validation is
        attempted and a section of parameters is ordered by the documentation
        itself and its sources.
    **options
        Any argument `docwrap` accepts, meaning the same thing there. See
    ...
    -------
    str
        The composed documentation, rendered in `render` when that was given,
        and otherwise in the format the documentation itself is written in.
    ...
    DocMappingError
        If a mapping or a bound source is invalid.
    """
    _, doc, composed = _standalone(obj, options, 'docrender')
    result = doc if composed is None else composed
    render = options.get('render', options.get('format'))
    return render_document(result, format=render)
